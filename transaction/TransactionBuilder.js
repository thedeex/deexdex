...
var _deexdexSerializer = require("deexdex-serializer");
...
class TransactionBuilder {
  ...
  process_transaction(cwallet, signer_pubkeys = null, broadcast = false) {
    ...
    if ((0, _deexdexApi.getConfig)().chainId !== wallet_object.get("chain_id")) return Promise.reject("Mismatched chain_id; expecting " + wallet_object.get("chain_id") + ", but got " + (0, _deexdexApi.getConfig)().chainId);
  ...
  finalize() {
    return new Promise((resolve, reject) => {
      ...
      resolve(_deexdexApi.database.getObjects(["2.1.0"]).then(r => {
        head_block_time_string = r[0].time;
        if (this.expiration === 0) this.expiration = base_expiration_sec() + (0, _deexdexApi.getConfig)().expireInSecs;
        ...
        this.tr_buffer = _deexdexSerializer.ops.transaction.toBuffer(this);
      }));
    });
  }
  ...
  id() {
    ...
    return _deexdexEcc.hash.sha256(this.tr_buffer).toString("hex").substring(0, 40);
  }
  /**
  ...
  get_type_operation(name, operation) {
    ...
    (0, _assert.default)(operation, "operation");
    var _type = _deexdexSerializer.ops[name];
    (0, _assert.default)(_type, `Unknown operation ${name}`);
    var operation_id = _deexdexSerializer.ChainTypes.operations[_type.operation_name];
    ...
    if (name === "proposal_create") {
      ...
      operation.expiration_time || (operation.expiration_time = base_expiration_sec() + (0, _deexdexApi.getConfig)().expireInSecsProposal);

      if (requiresReview) {
        operation.review_period_seconds = extraReview + Math.max(committee_min_review, 24 * 60 * 60 || (0, _deexdexApi.getConfig)().reviewInSecsCommittee);
        /*
  ...
  update_head_block() {
    return _deexdexApi.database.getObjects(["2.0.0", "2.1.0"]).then(function (res) {
      let [g, r] = res;
      head_block_time_string = r[0].time;
  ...
  set_required_fees(asset_id, removeDuplicates) {
    ...
    for (let i = 0, op; i < this.operations.length; i++) {
      ...
      let opObject = _deexdexSerializer.ops.operation.toObject(op);
    ...
    promises.push(Promise.all(feeAssets.map(id => {
      return _deexdexApi.database.getRequiredFees(operations, id);
    })).catch(err => {
    ...
    if (feeAssets.length > 1 || feeAssets[0] !== "1.3.0") {
      ...
      promises.push(_deexdexApi.database.getRequiredFees(operations, "1.3.0"));
      promises.push(_deexdexApi.database.getObjects(dynamicObjectIds));
    }
    ...
    return Promise.all(promises).then(results => {
      ...
      let set_fee = (operation, opIndex) => {
        if (!operation.fee || operation.fee.amount === 0 || operation.fee.amount.toString && operation.fee.amount.toString() === "0" // Long
            ...
            if (removeDuplicates) {
              let op = _deexdexSerializer.ops.operation.toObject(this.operations[opIndex]);
  ...
  get_potential_signatures() {
    var tr_object = _deexdexSerializer.ops.signed_transaction.toObject(this);

    return Promise.all([_deexdexApi.database.getPotentialSignatures(tr_object), _deexdexApi.database.getPotentialAddressSignatures(tr_object)]).then(function (results) {
      return {
        pubkeys: results[0],
        addys: results[1]
      };
  ...
  get_required_signatures(available_keys) {
    ...
    var tr_object = _deexdexSerializer.ops.signed_transaction.toObject(this); //DEBUG console.log('... tr_object',tr_object)


    return _deexdexApi.database.getRequiredSignatures(tr_object, available_keys).then(function (required_public_keys) {
  ...
  add_signer(private_key, public_key = private_key.toPublicKey()) {
    ...
    if (!public_key.Q) {
      public_key = _deexdexEcc.PublicKey.fromPublicKeyString(public_key);
    } // prevent duplicates
  ...
  sign(chain_id = (0, _deexdexApi.getConfig)().chainId) {
    if (!this.tr_buffer) {
      throw new Error("not finalized");
    }
    ...
    for (var i = 0; 0 < end ? i < end : i > end; 0 < end ? i++ : i++) {
      ...
      var sig = _deexdexEcc.Signature.signBuffer(_safeBuffer.Buffer.concat([new _safeBuffer.Buffer(chain_id, "hex"), this.tr_buffer]), private_key, public_key);
  ...
  serialize() {
    return _deexdexSerializer.ops.signed_transaction.toObject(this);
  }
...
function _broadcast(was_broadcast_callback) {
  return new Promise((resolve, reject) => {
    ...
    var tr_object = _deexdexSerializer.ops.signed_transaction.toObject(this); // console.log('... broadcast_transaction_with_callback !!!')


    _deexdexApi.network.broadcastTransactionWithCallback(function (res) {
      return resolve(res);
      ...
      reject(new Error(message + "\n" + "deex-crypto " + " digest " + _deexdexEcc.hash.sha256(this.tr_buffer).toString("hex") + " transaction " + this.tr_buffer.toString("hex") + " " + JSON.stringify(tr_object)));
      return;
    });
