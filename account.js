...
class Account {
  static async getAccout(_name) {
    ...
    let acc = await _deexdexApi.database.getAccountByName(name);
  ...
  static async id(id) {
    ...
    let acc = (await _deexdexApi.database.getAccounts([id]))[0];
  ...
  static async update() {
    let allData = await _deexdexApi.database.getAccounts(Object.keys(this.map).map(name => this.map[name].id));
  ...
  async update() {
    Object.assign(this, (await _deexdexApi.database.getAccounts([id]))[0]);
  }
