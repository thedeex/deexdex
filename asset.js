...
class Asset {
  static async getAsset(_symbol) {
    ...
    let obj = (await _deexdexApi.database.listAssets(symbol, 1))[0];
  ...
  static async id(id) {
    ...
    asset = (await _deexdexApi.database.getAssets([id]))[0];
  ...
  static async update() {
    let assets = await _deexdexApi.database.getAssets(Object.keys(this.map).map(symbol => this.map[symbol].id));
  ...
  async update() {
    Object.assign(this, (await _deexdexApi.database.getAssets([this.id]))[0]);
  }
