...
class Event {
  ...
  static async subscribeBlock() {
    await _deexdexApi.database.setSubscribeCallback(this.getUpdate.bind(this), false);
    await _deexdexApi.database.getObjects(["2.1.0"]);
  }

  static async subscribeAccount() {
    if (this.account.newSubs.length == 0) return;
    await _deexdexApi.database.getFullAccounts(this.account.newSubs, true);
    ...
    Object.keys(this.account.map).forEach(async accName => {
      ...
      if (!obj.history) obj.history = (await _deexdexApi.history.getAccountHistory(obj.id, "1.11.0", 1, "1.11.0"))[0].id;
    });
  }
  ...
  static async updateAccounts(ids) {
    ...
    for (let id of ids) {
      ...
      acc.events = await _deexdexApi.history.getAccountHistory(id, acc.history, 100, "1.11.0");
